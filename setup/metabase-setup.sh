...
# creating a postgres
curl -s -X POST http://$1/api/database -H 'Content-Type: application/json' --cookie "metabase.SESSION=$sessionToken" --data '{"engine":"clickhouse","name":"clickhouse","details":{"host":"clickhouse","port":8123,"user":"username","password":"password","dbname":"ecommerce","scan-all-databases":false,"ssl":false,"tunnel-enabled":false,"advanced-options":false},"is_full_sync":true}' &&
curl -s -X DELETE http://$1/api/database/1 -H 'Content-Type: application/json' --cookie "metabase.SESSION=$sessionToken" && \
curl -s -X PUT http://$1/api/table -H 'Content-Type: application/json' --cookie "metabase.SESSION=$sessionToken" --data '{"ids":[10],"visibility_type":"hidden"}'; fi